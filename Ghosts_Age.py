"""
Nicola takes a moment to study the ghosts. He is trying to think up a new method to determine just how old these
ghosts are. He knows that their age is somehow related to their opacity. To measure their opacity Nikola uses a scale
of 10000 units to 0 units, where 10000 is a completely opaque newborn ghost (0 years old) and 0 is completely
...
opacity is reduced by a number of units equal to its age when its age is a Fibonacci number or
...
def checkio(opacity):
    ...
    previous_fib = 0
    fib = 1
    fib_list = [previous_fib, fib]
    opacity_counter = 10000
    while opacity_counter != opacity:
        age += 1
        while fib < 5000:  # Further calculations of the Fibonacci series will be unnecessary.
            fib, previous_fib = fib + previous_fib, fib
            fib_list.append(fib)
        opacity_counter -= age if age in fib_list else -1
    return age, fib_list
